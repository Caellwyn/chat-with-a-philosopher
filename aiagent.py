...
class AIAgent():
    ...
    def query(self, prompt, temperature=.1):
        ...
        self.add_message(prompt + self.prefix, 'user')

        # Query the model through the API 
        self.result = self.agent.chat.completions.create(
            model=self.model,
            messages=self.history,
            temperature=temperature, # this is the degree of randomness of the model's output
        )

        self.response = self.result.choices[0].message.content
        ...
        self.prefix = 'Respond as the previous philosopher.  Do not change your persona: '
